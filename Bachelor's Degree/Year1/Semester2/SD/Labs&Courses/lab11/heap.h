...
MinHeapNode* removeMin(MinHeap* h)
{
    ...
    h->elem[0] = last;
    h->pos[min->v] = h->size-1; // intuiesc ca se face ca sa nu mai apara in minheap dupa ce scadem si size-ul heapului
    h->pos[last->v] = 0; // intuiesc ca se face ca sa apara in minheap pentru ca altfel fiind pe poz last->v ar fi sub h->size
    h->size--;
    SiftDown(h, 0);
...
void SiftUp(MinHeap* h, int v, int d)
{
   ...
   // printf("%d\n", i);
    h->elem[i]->d = d;
 //   printf("\n");
    while ((i >= 1) && d < h->elem[(i - 1) / 2]->d)
    {
   //     printf("%d %d |", h->elem[i]->v, h->elem[(i-1)/2]->v);
