...
void printHeightsAndElems_(AVLTree* tree, AVLNode* node) {
	...
	printHeightsAndElems_(tree, node->l);
	printHeightsAndElems_(tree, node->r);
	printf("valoarea elementului este: %d si inaltimea este: %d\n", *(node->elem), node->height);
	return;
}

void printHeightsAndElems(AVLTree* tree) {
	printHeightsAndElems_(tree, tree->root->l);
}
...
int main(int argc, char* argv[]) {
	...
	//Item elem = tree->root->l->r->elem;
	//printf("%ld\n", *elem);
	printHeightsAndElems(tree);
