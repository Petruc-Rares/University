...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	/* Dacă vreți ca pachetul de ICMP să conțină date in
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	icmp_hdr->code = 0;
	icmp_hdr->un.echo.id = getpid();
	for(int hop_no = 1; hop_no <=64; hop_no++) {
		ip_hdr->ttl = hop_no;
		ip_hdr->check = 0;
		ip_hdr->check = checksum(ip_hdr, sizeof(struct iphdr));

		icmp_hdr->un.echo.sequence = hop_no;
		icmp_hdr->checksum = 0;
		icmp_hdr->checksum = checksum(icmp_hdr, sizeof(struct icmphdr));
		...
		struct in_addr adr;
		adr.s_addr = rip_hdr->saddr;
		printf("Reply from %s with type %d\n",inet_ntoa(adr), ricmp_hdr->type);
		if(ricmp_hdr->type == ICMP_ECHOREPLY){
			break;
		}
	}
