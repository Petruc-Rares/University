...
int receive_and_send(int connfd1, int connfd2) {
    ...
    DIE(bytes_received < 0, "Received failed");

    fprintf(stderr, "Received: %s", buf);

    bytes_remaining = bytes_received;

    // TODO 7: Trimiteti mesajul la connfd2
    bytes_sent = send(connfd2, buf, bytes_remaining, 0);
...
void run_chat_server(int listenfd) {
    ...
    // TODO 4: Ascultati pentru doi clienti pe socketul dat
    int rc = listen(listenfd, 2);
    DIE(rc < 0, "Listen fail");
