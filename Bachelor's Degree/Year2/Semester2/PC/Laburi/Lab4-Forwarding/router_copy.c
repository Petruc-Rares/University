...
struct route_table_entry *get_best_route(__u32 dest_ip) {
	...
	for (int i = 0; i < rtable_size; i++) {
		if ((rtable[i].mask & dest_ip) == rtable[i].prefix) {
			// daca nu s-a gasit nicio ruta pana acum sau
			// gasim o masca mai restrictiva, updatam noua ruta
			if ((bestIndex == -1) || (rtable[i].mask < ntohl(rtable[bestIndex].mask))) {
				bestIndex = i;
			}
		}
	}
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		if (ip_checksum(ip_hdr, sizeof(struct iphdr)) != 0) {
			fprintf(stderr, "Bad checksum, dropping packet");
			continue;
		}
		...
		/* TODO 4: Check TTL >= 1 */
		if (ip_hdr->ttl < 1) {
			fprintf(stderr, "TTL is %d, dropping packet!\n", ip_hdr->ttl);
			continue;
		}
		...
		fprintf(stderr, "Sending packet on interface %d\n", best_route->interface);
