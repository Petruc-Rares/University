...
int find_after_sock(Node *list, int sock) {
    while (list) {
        if (list->content.socket_client == sock) {
            return list->content.number_generated;
        }
        list = list->next;
    }
    return -1;
...
Node *createNode(int sock_fd, int number_generated) {
    Node *newNode = malloc(sizeof(Node));
    newNode->content.socket_client = sock_fd;
    newNode->content.number_generated = number_generated;
    newNode->next = NULL;
