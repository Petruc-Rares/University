...
(define (enqueue x q)
  ; daca deja stivele sunt egale, inseamna ca in urma noului push
  ; se va ajunge ca stiva dreapta sa contina mai multe elemente decat cea stanga
  (if (= (queue-size-r q) (queue-size-l q))
      (make-queue (rotate (queue-left q) (cons x (queue-right q)) empty-stream) null (add1 (+ (queue-size-r q) (queue-size-l q))) 0)
...
(define (dequeue q)
  ; daca deja stivele sunt egale, inseamna ca in urma noului pop
  ; se va ajunge ca stiva din stanga sa scada deci pierdem invariantul
  (if (= (queue-size-r q) (queue-size-l q))           
      (make-queue (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream) null (sub1 (+ (queue-size-r q) (queue-size-l q))) 0)
...
(define (top q)
  (if (= (queue-size-l q) 0)
      ; daca nu putem extrage din prima stiva incearca din a doua
      (last (queue-right q))
      ; altfel returneaza primul element din prima
