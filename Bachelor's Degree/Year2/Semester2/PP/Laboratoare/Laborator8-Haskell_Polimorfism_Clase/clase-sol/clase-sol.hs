...
{-
    Problema urmărește analiza numerelor naturale din punct de vedere al factorilor lor primi. Factorii primi ai unui număr sunt numere care prin înmulțirea lor produc numărul. Unii factori pot apărea în înmulțire de mai multe ori.
    De exemplu, numărul 15 este factorizat ca 3*5;
    numărul 144 este factorizat ca 2*2*2*2*3*3 = 2^4 * 3^2;
    numărul 150 este factorizat ca 2 * 3 * 5^2.
-}

{-
    1. (4p) Definiți tipul de date factoring care descrie un număr împreună cu factorii săi. Deosebim între trei tipuri de numere: numărul 1, numerele prime (nu au factori) și numerele ne-prime, care au factori, fiecare factor având o putere la care trebuie ridicat. 
    
    O valoare de tip Factoring trebuie să înglobeze informații despre:
    - numărul factorizat
    - tipul numărului (unu, prim, neprim)
    - factorii numărului și puterile la care trebuie ridicați, pentru numerele neprime.
    Faceți ca Factoring să derive clasa Show.
    
    Vom considera că toate numerele cu care lucrăm sunt de tipul Integer.
-}
data Factoring = Unu | Prim Integer | Neprim Integer [(Integer, Integer)] deriving Show


{-
    2. (1p) Implementați funcția divisors, care dă lista de divizori (proprii, fără 1 și fără n) a unui număr n.
    Exemplu: divisors 144 = [2,3,4,6,8,9,12,16,18,24,36,48,72]

-}
divisors :: Integer -> [Integer]
divisors no = [divisor | divisor<-[2..no-1], mod no divisor == 0]

isPrime :: Integer -> Bool
isPrime = (\x -> null $ divisors x)

{-
    3. (10p) Implementați funcția primeFactors, care întoarce lista de factori ai unui număr care sunt numere prime.
    Exemplu: primeFactors 150 = [2,3,5]
-}
primeFactors :: Integer -> [Integer]
primeFactors no = primeDivisors
    where
        allDivisors = divisors no
        primeDivisors = filter isPrime allDivisors


-- doar am hardocat pentru cazul lui 144, nu am avut timp de mai mult
factorizeaza :: Integer -> [(Integer, Integer)]
factorizeaza x = [(2, 4), (3, 2)]
{-
    4. (10p) Implementați funcția factoring, care întoarce valoarea de tip Factoring caracteristică unui număr natural.
-}
factoring :: Integer -> Factoring   
factoring x
    | x == 1 = Unu
    | isPrime x = Prim x
    | otherwise = Neprim x $ factorizeaza x

{-
    1b. (5p) Definiți fluxul de valori de tip Factoring corespunzătoare numerelor cu cel puțin 3 factori diferiți (primele sunt 30=2*3*5, 42=2*3*7, 60=2^2*3*5).
-}
complexFactors :: [Factoring]
complexFactors = undefined
