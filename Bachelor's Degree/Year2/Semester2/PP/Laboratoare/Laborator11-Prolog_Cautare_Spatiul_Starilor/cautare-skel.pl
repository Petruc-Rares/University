...
bfs([(CrtNode, Parent)|Rest], Closed, Solution) :- 
    \+member((CrtNode, Parent), Closed),
    getNeighb(CrtNode, [], Accesibles),
    append(Rest, Accesibles, NewAccessibles),
    bfs(NewAccessibles, [(CrtNode, Parent)|Closed], Solution).
...
med(L, Med):- member(Med, L),
            findall(Y, (member(Y, L), Y > Med), L1),
            findall(Z, (member(Z, L), Z < Med), L2),
            length(L1, Length1), length(L2, Length2),
            ((Length1 = Length2); (Length1 =:= Length2 - 1);
            (Length1 =:= Length2 + 1)).


extract_path_aux(_, [CrtHead| Rest], [CrtHead | Rest]):- initial_node(CrtHead), !.

extract_path_aux(Discovered, [CrtHead| Rest], Solution) :-
     member((CrtHead, CrtParentHead), Discovered),
     extract_path_aux(Discovered, [CrtParentHead, CrtHead | Rest], Solution).
extract_path([(FinalNode, Parent)|Rest], RestSolution):- final_node(FinalNode),
        extract_path_aux([(FinalNode, Parent)|Rest], [FinalNode], RestSolution).

appendList(_, 0, Sol, Sol):- !.
appendList(List, TimesReplicate, List2, Sol):- 
    AnotherReplicate is TimesReplicate - 1,
    append(List2, List, List2_),
    appendList(List, AnotherReplicate, List2_, Sol).
    
appendPartList(_, Sol, 0, Sol):- !.
appendPartList([X|Xs], PartialSol, Rest, Sol):-
    Rest_ is Rest - 1,
    append(PartialSol, [X], PartialSol_),
    appendPartList(Xs, PartialSol_, Rest_, Sol).

minmax(L, Min, Max):-
    member(Min, L), member(Max, L),
    findall(X, (member(X, L), X > Min, X < Max), List),
    length(L, LL),
    L__ is LL - 2,
    length(List, L__).
...
preorder_([], []).
preorder_([Node|Rest], [Node|Park]):- \+arc(Node, _), preorder_(Rest, Park).
preorder_([Node|Rest], [Node|Park]):- arc(Node, Children), append(Children, Rest, NewChildren),
                                      preorder_(NewChildren, Park).

preorder(Node, Park) :- preorder_([Node], Park).
...
trees(Trees) :- nodes([], Result), parcurgeri(Result, Trees).

parcurgeri([], []).
parcurgeri([Head|Rest], [OneTree|RestTrees]):- preorder(Head, OneTree),
                                                setMinus(Rest, OneTree, Remain),
                                                parcurgeri(Remain, RestTrees).
...
a(_, [], _, []).
a(_ , _, [], []).
a([X|Xs], [Y|Ys], [Z|Zs], [(X, Y, Z)|RestSol]):-
    a(Xs, Ys, Zs, RestSol).
...
secondMin(L, X):-
    member(X, L),
    length(L, LL),
    findall(Y, (member(Y, L), Y > X), LY),
    L_ is LL - 2,
    length(LY, L_).

mulHelper(X, RestList):- forall(member(Y, RestList), X mod Y =:= 0).

mul([], []).
mul([X|Xs], [X|RestSol]):- mulHelper(X, Xs), !, mul(Xs, RestSol).
mul([_|Xs], RestSol):- mul(Xs, RestSol).

secondMax(L, Max):- member(Max, L),
                    findall(Y, (member(Y, L), Y > Max), [_]).

one_bubble([], []).
one_bubble([X], [X]).
one_bubble([X, Y|Rest], [X|RestSol]):- X < Y, one_bubble([Y|Rest], RestSol).
one_bubble([X, Y|Rest], [Y|RestSol]):- Y < X, one_bubble([X|Rest], RestSol).
