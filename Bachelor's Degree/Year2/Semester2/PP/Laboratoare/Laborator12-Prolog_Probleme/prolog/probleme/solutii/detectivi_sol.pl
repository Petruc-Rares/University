...
find2Occ(L, X):-
                ...
                \+member(X, A),
                append(B, [X | C], Rest),
                \+member(X, B), \+member(X, C).

solve(L1, L2, Res):- findall(X, (member(X, L1), findall(Y, (member(Y, L2), 0=:=mod(X, Y)), [_])), Res).


noRedundaciesHelper([], _, Sol, FinalSol):- reverse(Sol, FinalSol).
noRedundaciesHelper([CrtList|RestLists], AccElems, AccCrtLists, Sol):- subset(CrtList, AccElems), !,
                                                            noRedundaciesHelper(RestLists, AccElems, AccCrtLists, Sol).
noRedundaciesHelper([CrtList|RestLists], AccElems, AccCrtLists, Sol):-
                                                            append(AccElems, CrtList, NextAccElems),
                                                            sort(NextAccElems, NextAccElems_),
                                                            noRedundaciesHelper(RestLists, NextAccElems_, [CrtList| AccCrtLists], Sol).

noRedundancies(LLIn, LLOut):-
    noRedundaciesHelper(LLIn, [], [], LLOut).

singles(L, X):- findall(Y, (member(Y, L), findall(Z, (member(Z, L), Z = Y), [_])), PartialAnswer), sort(PartialAnswer, X).
%% singles([8,1,2,3,1,2,5,4,7,6,2,5], X) -> [3, 4, 6, 7, 8]