...
(define (replicate-helper n L acc)
 (cond 
    ((null? L) acc)
    (replicate-helper n (cdr L) (replicate-helper n (cdr L) (append acc (make-list n (car L)))))))

(define (replicate n L)
  (replicate-helper n L null))

(replicate 2 '(10 11 12 13))

 (flatten (map (λ (x) (make-list 2 x)) '(10 11 12 13))) ;am reusit sa-l fac si asa
;(trace app-iter)
;(app '(1 2 3 4 5) '(a b c))
