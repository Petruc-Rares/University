...
(define (compute-length get-line-segment get-start-point get-end-point)
    (let* ((segment (get-line-segment))
          (primul (get-start-point segment))
          (ultimul (get-end-point segment)))
    (sqrt (+ (sqr (- (car primul) (car ultimul))) (sqr (- (cdr primul) (cdr ultimul)))))))
...
(define (distance x y)
  (compute-length (λ() (cons x y)) get-start-point get-end-point))
...
(define (compute-f-with-step f a b step)
  (let helper ((a a)
    ...
    (if (> a b)
        (reverse result)
        (helper (+ a step) b (cons (f a) result)))))
;   (let iter ((b b) (result '()))             
;   (if (> a b)                                
;       result  
...
(define (num-concat x y)
  (define (+ x y)
    (let ((x (number->string x))
          (y (number->string y))) ; aici suntem in zona de definiții, nu în corpul let-ului =\> a e legat la 10
...
(define (compute-perimeter points)
  ...
  (let iter ((pointsLeft points) (perimeter 0))            
   (if (= 1 (length pointsLeft))                             
       (+ perimeter (distance (car pointsLeft)  start)) 
       (iter (cdr pointsLeft) (+ perimeter (distance (car pointsLeft) (cadr pointsLeft))))))))
