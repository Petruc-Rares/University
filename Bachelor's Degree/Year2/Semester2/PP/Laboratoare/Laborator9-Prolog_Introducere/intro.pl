...
myConcat([],L2,L2).
myConcat([X|XS],L2,[X|L]) :- myConcat(XS, L2, L).
...
myReverse([],[]).
myReverse([X|XS],ReversedList):- myReverse(XS,XSReversed), myConcat(XSReversed,[X],ReversedList).
...
myReverseAcc([X|Xs], Acc, RevList):- myReverseAcc(Xs, [X|Acc], RevList).
...
factorial(0, 1).
factorial(N, F):- N_ is N-1, factorial(N_, F_), F is N * F_.
...
setIntersection([], _, []). 
setIntersection([X|Xs], L2, [X|RestIntersect]):- member(X, L2), setIntersection(Xs, L2, RestIntersect).
setIntersection([X|Xs], L2, RestIntersect):- not(member(X, L2)), setIntersection(Xs, L2, RestIntersect).
...
setDiff([], _, []).
setDiff([X|Xs], L2, [X|RestDiff]):- not(member(X, L2)), setDiff(Xs, L2, RestDiff).
setDiff([X|Xs], L2, RestDiff):- member(X, L2), setDiff(Xs, L2, RestDiff).
...
setUnion(L1, L2, Union):- setIntersection(L1, L2, Intersection), setDiff(L2, Intersection, Diff), myConcat(L1, Diff, Union).
...
descendantOf(X, Y):- nod(X), nod(Y), (arc(Y, X); arc(Z, X), descendantOf(Z, Y)).
...
sameTree(X, Y):- isRoot(Z), descendantOf(X, Z), descendantOf(Y, Z).
...
cost(A, B, N2):- arc(C, A), cost(C, B, N), N2 is N - 1.
