...
myConcat([], L2, L2).
myConcat([X|Xs], L2, [X|RestL]):- myConcat(Xs, L2, RestL).
...
myReverse([], []).
%%myReverse([X|Xs], [RestSol|X]):- myReverse(Xs, RestSol).
myReverse([X|Xs], Sol):- myReverse(Xs, SolAux), myConcat(SolAux, [X], Sol).
...
myReverseAcc([], Sol, Sol).
myReverseAcc([X|Xs], Acc, Sol):- myReverseAcc(Xs, [X|Acc], Sol).
...
setIntersection([], _, []).
setIntersection([X|Xs], L2, [X|RestSol]):- member(X, L2), !, setIntersection(Xs, L2, RestSol).
setIntersection([_|Xs], L2, RestSol):- setIntersection(Xs, L2, RestSol).

listIntersection([], _, _, []).
listIntersection([X|Xs], L2, Acc, [X|RestSol]):- (member(X, L2) , \+member(X, Acc)), !, listIntersection(Xs, L2, [X|Acc], RestSol).
listIntersection([_|Xs], L2, Acc, RestSol):- listIntersection(Xs, L2, Acc, RestSol).
%%listIntersection([_|Xs], L2, Acc, Acc).
...
setDiff([], _, []).
setDiff([X|Xs], L2, [X|RestSol]):- \+member(X, L2), !, setDiff(Xs, L2, RestSol).
setDiff([_|Xs], L2, RestSol):- setDiff(Xs, L2, RestSol).
...
setUnion_([], L2, Acc, Sol):- reverse(Acc, Acc_),append(Acc_, L2, Sol).
setUnion_([X|Xs], L2, Acc, RestSol):- (\+member(X, Acc), \+member(X, L2)), !, setUnion_(Xs, L2, [X|Acc], RestSol).
setUnion_([_|Xs], L2, Acc, RestSol):- setUnion_(Xs, L2, Acc, RestSol).
setUnion(L1, L2, Sol):- setUnion_(L1, L2, [], Sol).
...
%%descendantOf(X, Y):- descendantOf(Z, Y), arc(Z, X).
...
sum([], 0).
sum([X|Xs], SRest):- sum(Xs, SRest_), SRest is SRest_ + X. 
