...
permutari4([A, B, C, D], Permutare4) :- length(Permutare4, 4),
                                        member(A, Permutare4), member(B, Permutare4), member(C, Permutare4), member(D, Permutare4).
...
alternare(_, _, 0, []) :- !.
alternare(A, B, Len, [A|RestRes]) :-  0 =:= mod(Len,2), Len_ is Len - 1, alternare(A, B, Len_, RestRes).
alternare(A, B, Len, [B|RestRes]) :-  1 =:= mod(Len,2), Len_ is Len - 1, alternare(A, B, Len_, RestRes).
...
sublista([], []).
sublista([X|RestX], [Y|RestY]):- X=Y, sublista(RestX, RestY).
sublista([_|RestX], Y):- sublista(RestX, Y).
...
prieteni(mo, momo).
prieteni(momo, momomo).
prieteni(momomo, momomomo).
prietenos(A) :- prieteni(A,_); prieteni(_, A).

%prieteni(dorel, gigi).
%prieteni(gigel, andrei).
%prieteni(A, B) :- prieteni(B, A).
%prieteni(A, B) :- prieteni(A, X), prieteni(X, B).
...
%extract(List, _, End, []) :- length(List, Lung), Lung > End.
extract([], _, _, []).
extract(_, _, -1, []).
extract([_|List], Start, End, Range):- Start > 0, Start_ is Start - 1, End_ is End - 1, extract(List, Start_, End_, Range).
extract([X|List], Start, End, [X|Range]):- Start = 0, End >= 0, End_ is End - 1, extract(List, Start, End_, Range).
...
factorial2(0, 1).
factorial2(N, Fact):- factorial2(N_, Fact_), N is N_ + 1, Fact is N*Fact_.
...
auCopil(X, Y) :- X \= Y, parinte(Z, X), parinte(Z, Y), !.
...
soti(X, Y) :- persoane(L), member(X, L), member(Y, L), X @< Y, auCopil(X, Y).
...
linie(X, [X]) :- \+parinte(X, _).
linie(X, [X|Rest]) :- parinte(X, Y), linie(Y, Rest).
...
%prima_linie_prin(Urmas, Persoana, Linie) :- linie(Urmas, L), member(Persoana, L), !, L = Linie.
...
stramosi(Urmas, [Urmas| Rest]) :- parinte(Urmas, X), parinte(Urmas, Y), X \= Y, !,
                                    stramosi(X, RestX), stramosi(Y, RestY), append(RestX, RestY, Rest).
