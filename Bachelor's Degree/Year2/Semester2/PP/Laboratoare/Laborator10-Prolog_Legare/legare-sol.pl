...
extract([], _, _, []):- !.
extract(_, _, -1, []):- !.
extract([X|Xs], 0, End, [X|RestSol]):- End_ is End - 1, extract(Xs, 0, End_, RestSol).
extract([_|Xs], Start, End, Range):- Start > 0, End > 0, Start_ is Start - 1, End_ is End - 1, extract(Xs, Start_, End_, Range).
